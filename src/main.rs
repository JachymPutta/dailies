...
use crate::keywords::*;
use crate::mdast::{mdast_to_string, replace_pattern};
...
fn update_template(config: &Config) -> String {
    ...
    if let Ok(contents) = read_to_string(&config.entry_template) {
        if let Ok(mut parsed) = markdown::to_mdast(&contents, &markdown::ParseOptions::gfm()) {
                ...
                config.get_previous_daily()
            {
                ...
                update_todos(&parsed, &previous_path)
            } else {
                mdast_to_string(&parsed)
            }
        } else {
            eprintln!(
